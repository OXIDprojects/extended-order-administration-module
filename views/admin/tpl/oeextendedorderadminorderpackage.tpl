[{include file="headitem.tpl" title="OEEXTENDEDORDERADMIN_TITLE"|oxmultilangassign box=" "}]
...
<br>
<div id="noprint"><br>
<a class="listitem" href="javascript:printWindow();"><b>[{oxmultilang ident="OEEXTENDEDORDERADMIN_SHOWPACKLIST"}]</b></a><br>
<br><br></div>

<span class="listitem">
<b>[{oxmultilang ident="OEEXTENDEDORDERADMIN_PACKLIST"}]</b><br><br>
</span>
...
<tr>
    <td class="listitem" width="150" valign="top">
        ...
        [{oxmultilang ident="OEEXTENDEDORDERADMIN_FON"}]: [{$order->oxorder__oxbillfon->value}]<br>
        [{oxmultilang ident="OEEXTENDEDORDERADMIN_FAX"}]: [{$order->oxorder__oxbillfax->value}]<br>
        [{if $order->oxorder__oxremark->value}]<b><br>[{oxmultilang ident="OEEXTENDEDORDERADMIN_REMARK"}]: <br>[{$order->oxorder__oxremark->value}]</b><br>[{/if}]
    </td>
    <td class="listitem" width="150" valign="top">
        ...
        [{oxmultilang ident="OEEXTENDEDORDERADMIN_FON"}]: [{if $order->oxorder__oxdelfon->value}][{$order->oxorder__oxdelfon->value}][{else}][{$order->oxorder__oxbillfon->value}][{/if}]<br>
        [{oxmultilang ident="OEEXTENDEDORDERADMIN_FAX"}]: [{if $order->oxorder__oxdelfax->value}][{$order->oxorder__oxdelfax->value}][{else}][{$order->oxorder__oxbillfax->value}][{/if}]<br>
    </td>
    <td class="packitem" valign="top">
    [{oxmultilang ident="OEEXTENDEDORDERADMIN_ORDERNR1"}][{$order->oxorder__oxordernr->value}] - [{oxmultilang ident="OEEXTENDEDORDERADMIN_ORDERNR2"}] [{$order->oxorder__oxorderdate->value|oxformdate:"datetime":true}]<br><br>
                            ...
                            [{foreach key=sVar from=$article->getPersParams() item=aParam name=persparams}]
                                [{if $aParam}]
                                    ...
                                    [{if $smarty.foreach.persparams.first && $smarty.foreach.persparams.last}]
                                        [{oxmultilang ident="OEEXTENDEDORDERADMIN_DETAILS"}]
                                    [{else}]
                                        [{$sVar}] :
                                    [{/if}]
                                    [{$aParam}]
...
{   parent.parent.sShopTitle   = "[{$actshopobj->oxshops__oxname->getRawValue()|oxaddslashes}]";
    parent.parent.sMenuItem    = "[{oxmultilang ident="OEEXTENDEDORDERADMIN_MENUITEM"}]";
    parent.parent.sMenuSubItem = "[{oxmultilang ident="OEEXTENDEDORDERADMIN_MENUSUBITEM"}]";
